    ...
    private fun readMovies(databaseFile: File): List<MovieEntity> {
        ...
        return try {
            database.rawQuery("SELECT * FROM movies ORDER BY id", null).use { cursor ->
                ...
                val ratingColumn = cursor.optionalColumn("personal_rating", "personalRating")
                val favoriteColumn = cursor.optionalColumn("favorite")
                ...
                while (cursor.moveToNext()) {
                    ...
                    movies += MovieEntity(
                        ...
                        season = cursor.intAt(seasonColumn).coerceAtLeast(1)
                            .takeIf { cursor.intAt(seriesColumn) != 0 } ?: 0,
                        episode = cursor.intAt(episodeColumn).coerceAtLeast(1)
                            .takeIf { cursor.intAt(seriesColumn) != 0 } ?: 0,
                        ...
                        customOrder = cursor.intAt(customOrderColumn)
                            ,
                        personalRating = cursor.intAt(ratingColumn)
                            .takeIf { ratingColumn >= 0 }
                            ?.coerceIn(1, 5),
                        favorite = cursor.intAt(favoriteColumn) != 0
                    )
                }
